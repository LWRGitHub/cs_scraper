...
# set old data
r = requests.get(f"https://sfbay.craigslist.org/search/zip?query=mac&search_distance=100&postal={ZIP_TO_SEARCH_AROUND}")
soup = bs(r.content)
...
while True:
    ...
    if old_data != h3:
        ...
        if there_is_a_new_href:
            ...
            if message_to_send != '':
                emojionearea_editor.send_keys(f"https://sfbay.craigslist.org/search/zip?query=mac&search_distance=100&postal={ZIP_TO_SEARCH_AROUND} Check Craigslist Mac stuff! {message_to_send}")
            else:
                emojionearea_editor.send_keys(f'Something is wrong with Craigslist Mac bot https://sfbay.craigslist.org/search/zip?query=mac&search_distance=100&postal={ZIP_TO_SEARCH_AROUND}')
