...
# set old data
r = requests.get(f"https://trashnothing.com/beta/browse?search=laptop&radius={RADIAOUS}")
soup = bs(r.content)
...
while True:
    ...
    if old_data != recent_data:
        ...
        if there_is_a_new_href:
            ...
            if message_to_send != '':
                emojionearea_editor.send_keys(f"https://trashnothing.com/beta/browse?search=laptop&radius={RADIAOUS} Check Trash Nothing 'Laptop' stuff! {message_to_send}")
            else:
                emojionearea_editor.send_keys(f'Something is wrong with Trash Nothing "Laptop" bot https://trashnothing.com/beta/browse?search=laptop&radius={RADIAOUS}')
