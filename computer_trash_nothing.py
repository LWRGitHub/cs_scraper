...
# set old data
r = requests.get(f"https://trashnothing.com/beta/browse?search=computer&radius={RADIAOUS}")
soup = bs(r.content)
...
while True:
    ...
    if old_data != recent_data:
        ...
        if there_is_a_new_href:
            ...
            if message_to_send != '':
                emojionearea_editor.send_keys(f"https://trashnothing.com/beta/browse?search=computer&radius={RADIAOUS} Check Trash Nothing 'Computer' stuff! {message_to_send}")
            else:
                emojionearea_editor.send_keys(f'Something is wrong with Trash Nothing "Computer" bot https://trashnothing.com/beta/browse?search=computer&radius={RADIAOUS}')
